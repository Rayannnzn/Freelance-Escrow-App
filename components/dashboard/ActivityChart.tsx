    ...
    <div className="glass rounded-xl px-4 py-3 border border-white/10">
      ...
      {payload.map((p) => (
        <div key={p.dataKey} className="text-sm font-semibold text-foreground">
          {p.dataKey === 'volume' ? `${p.value} SOL` : `${p.value} contracts`}
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Global transaction volume across all active contracts
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            <span className="size-2 rounded-full bg-[#39ff14]" />
            <span className="label-caps text-[10px]">Volume</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <AreaChart data={MOCK_CHART_DATA} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
          <defs>
            ...
            <linearGradient id="gradBlue" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#00eefc" stopOpacity={0.2} />
